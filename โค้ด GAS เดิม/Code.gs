...
const CONSENT_TEMPLATE_ID = '1mXw6MdoAz0NMdZ2o51jlrGhe3i3uGQm5i8vCZ7Z4Z0';
const SOAP_TEMPLATE_ID = '1q_DBudqfmr_C8eiPjz5WdXb4QTdKrT6RPf-uJQYkxKU';

const TEMPLATE_FOLDER_ID = '1JJCfofS-zIaFZdA4JPzOnGUV-fhNQwfq';
...
function getPatientById(id) {
  try {
    ...
    if (patientObj.PatientPhotoURL) {
      try {
        const photoBlob = getImageBlob_(patientObj.PatientPhotoURL);
        patientObj.PatientPhotoBase64 = photoBlob
          ? `data:${photoBlob.getContentType()};base64,${Utilities.base64Encode(photoBlob.getBytes())}`
          : null;
      } catch (e) {
...
function isImagePlaceholderKey_(key) {
  return [
    'PatientPhoto',
    'BodyChartDrawing',
    'TherapistSignature',
...
/**
 * แปลงค่ารูปภาพเป็น Blob สำหรับ Google Docs
 * รองรับ Google Drive URL/ID, data URI และ HTTPS URL เช่น Supabase Storage.
 */
function getImageBlob_(value) {
  ...
  if (/^data:image\//i.test(raw)) {
    try {
      ...
      const header = raw.substring(5, raw.indexOf(';')) || 'image/png';
      const bytes = Utilities.base64Decode(raw.substring(commaIndex + 1));
      return Utilities.newBlob(bytes, header, 'image');
    } catch (e) {
      Logger.log('Unable to decode image data URI: ' + e);
      return null;
    }
  }
  ...
  if (fileId) {
    try {
      ...
      Logger.log('Unable to read Drive image ' + fileId + ': ' + e);
    }
  }

  // 3) Supabase Storage / any publicly reachable HTTPS image
  if (/^https?:\/\//i.test(raw)) {
    try {
      ...
      if (code >= 200 && code < 300) {
        ...
        if (blob && String(blob.getContentType() || '').toLowerCase().indexOf('image/') === 0) {
          return blob;
        }
        // Some storage/CDN endpoints do not return an image MIME type correctly.
        return blob;
      }
      Logger.log('Image URL returned HTTP ' + code + ': ' + raw);
    } catch (e) {
      Logger.log('Unable to fetch image URL ' + raw + ': ' + e);
    }
  }
...
function replaceTextExact_(body, placeholder, value) {
  body.replaceText(escapeRegExp_(placeholder), String(value == null || value === '' ? ' ' : value));
}
...
function generatePdfAsBase64(templateId, data, fileNamePrefix) {
  ...
  try {
    data = data || {};
    const folderId = (typeof TEMPLATE_FOLDER_ID !== 'undefined' && TEMPLATE_FOLDER_ID) ? TEMPLATE_FOLDER_ID : PHOTO_FOLDER_ID;
    ...
    // Consent template uses Thai labels rather than the normalized keys.
    if (templateId === CONSENT_TEMPLATE_ID) {
      replaceTextExact_(body, '{{ลายมือชื่อ}}', '{{ConsenterSignature}}');
      replaceTextExact_(body, '{{ลายมือชื่อพยาน}}', '{{WitnessSignature}}');
    }

    // Replace text / checkbox placeholders. Regex metacharacters in keys such as
    // Fx.HIP are escaped so the placeholder is matched literally.
    Object.keys(data).forEach(key => {
      if (isImagePlaceholderKey_(key)) return;
      const placeholder = `{{${key}}}`;
      const value = data[key];

      if (value === true || value === '☑') {
        replaceTextExact_(body, placeholder, '☑');
      } else if (value === false || value === '☐') {
        replaceTextExact_(body, placeholder, '☐');
      } else {
        replaceTextExact_(body, placeholder, value);
      }
    });

    // Replace image placeholders from URL/ID/data URI.
    ...
    imagePlaceholders.forEach(key => {
      const urlProperty = data[key + 'Url'] || data[key + 'URL'] || data[key + 'Base64'];
      let blob = getImageBlob_(urlProperty);

      // Body Chart always has a safe original-template fallback.
      if (!blob && key === 'BodyChartDrawing') {
        try {
          blob = DriveApp.getFileById(BODY_CHART_IMAGE_ID).getBlob();
        } catch (e) {
          Logger.log('Unable to load original Body Chart fallback: ' + e);
        }
      }
      ...
      if (blob) {
        ...
        replacePlaceholderWithImageBlob_(body, `{{${key}}}`, blob, imageWidth);
      }
    });

    // Remove any placeholders that were not supplied by the adapter.
    body.replaceText('\\{\\{[^}]*\\}\\}', ' ');
    ...
    if (tempDocFile) {
      try { tempDocFile.setTrashed(true); } catch (cleanupError) { Logger.log('PDF temp cleanup failed: ' + cleanupError); }
    }
  }
}

/**
 * แทนที่ Placeholder ในเอกสารด้วย Blob รูปภาพ
 */
function replacePlaceholderWithImageBlob_(body, placeholder, blob, width) {
  try {
    ...
    replaceTextExact_(body, placeholder, '(ไม่สามารถโหลดรูปได้)');
    return false;
  }
}

// Backward-compatible helper retained for older callers.
function replacePlaceholderWithImage(body, placeholder, fileIdOrUrl, width) {
  const blob = getImageBlob_(fileIdOrUrl);
  if (!blob) {
    replaceTextExact_(body, placeholder, '(ไม่สามารถโหลดรูปได้)');
    return false;
  }
  return replacePlaceholderWithImageBlob_(body, placeholder, blob, width);
}
...
function generateTMSEPdf(recordId) {
  const recordResult = getRecordById(tmseSheet, recordId, 'RecordID');
  const record = recordResult && recordResult.status === 'success' ? recordResult.record : null;
  const patient = record ? getPatientById(record.PatientID) : null;
...
function generateMHQPdf(recordId) {
  const recordResult = getRecordById(mhqSheet, recordId, 'RecordID');
  const record = recordResult && recordResult.status === 'success' ? recordResult.record : null;
  const patient = record ? getPatientById(record.PatientID) : null;
  if (!record || !patient) return { status: 'error', message: 'ไม่พบข้อมูล' };
...
function generateDysphagiaPdf(recordId) {
  const recordResult = getRecordById(dysphagiaSheet, recordId, 'RecordID');
  const record = recordResult && recordResult.status === 'success' ? recordResult.record : null;
  const patient = record ? getPatientById(record.PatientID) : null;
  if (!record || !patient) return { status: 'error', message: 'ไม่พบข้อมูล' };
...
function generateConsentPdf(consentId) {
  const consentResult = getRecordById(consentSheet, consentId, 'ConsentID');
  const consent = consentResult && consentResult.status === 'success' ? consentResult.record : null;
  const patient = consent ? getPatientById(consent.PatientID) : null;
...
function generateBIPdf(assessmentId) {
  const assessmentResult = getRecordById(biSheet, assessmentId, 'AssessmentID');
  const assessment = assessmentResult && assessmentResult.status === 'success' ? assessmentResult.record : null;
...
function generateOpdPdf(recordId) {
  const recordResult = getRecordById(opdSheet, recordId, 'RecordID');
  const record = recordResult && recordResult.status === 'success' ? recordResult.record : null;
  const patient = record ? getPatientById(record.PatientID) : null;
  if (!record || !patient) return { status: 'error', message: 'ไม่พบข้อมูล' };
